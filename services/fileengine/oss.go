...
func (o *OssStorage) getURL(fileType string, dir string) (signedURL string, err error) {
	...
	//}
	objectName := utils.GenerateUUIDShort22() + "." + fileType
...
func (o *OssStorage) PostSign(fileType string, dir string) *apimodels.PreSignResp {
	objectName := utils.GenerateUUIDShort22() + "." + fileType
	objectName = filepath.Join(dir, objectName)
	ret := &apimodels.PreSignResp{
		ObjKey: objectName,
	}
	...
	policy := utils.ToJson(policyMap)
	strToSign := base64.StdEncoding.EncodeToString([]byte(policy))
	ret.Policy = strToSign
	//signature
	h1 := utils.HmacSha256([]byte("aliyun_v4"+o.accessKeySecret), date)
	h2 := utils.HmacSha256(h1, o.region)
	h3 := utils.HmacSha256(h2, "oss")
	h4 := utils.HmacSha256(h3, "aliyun_v4_request")
	signature := hex.EncodeToString(utils.HmacSha256(h4, strToSign))
