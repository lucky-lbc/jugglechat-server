...
func (engine *SiliconFlowEngine) StreamChat(ctx context.Context, senderId, converKey string, prompt string, question string, f func(part string, isEnd bool)) {
	...
	stream, code, err := utils.CreateStream(http.MethodPost, engine.Url, headers, body)
	if err != nil || code != http.StatusOK {
		log.Errorf("call siliconflow api failed. http_code:%d,err:%v", code, err)
		return
	}
	for {
		...
		err = utils.JsonUnMarshal([]byte(line), &item)
		if err != nil {
			f("", true)
			return
		}
